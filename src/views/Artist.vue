                    ...
                    <div
                        ...
                        <div class="flex lg:flex-row flex-col items-center gap-14">
                            <div class="w-[250px] h-[250px] rounded-lg"
                                :style="{ backgroundImage: `url('${artistInfo?.images[0] ? artistInfo?.images[0].url : 'https://placehold.co/600x600'}')`, backgroundSize: 'cover' }">
                            </div>
                            <div class="flex flex-col gap-4">
                                <h2 class="text-white font-bold text-3xl">{{ artistInfo?.name }}</h2>
                                <p class="text-white font-medium text-md">{{
                                    artistInfo?.followers.total.toLocaleString('en-US') }} followers</p>
                        ...
                        <div v-if="topTracks?.length" class="flex flex-col gap-4 mt-10">
                            <h2 class="text-white font-bold text-xl">Top Tracks</h2>
                            <TrackCard :info="topTracks" :info-type="'track'"></TrackCard>
                        </div>
                        <div v-if="albums?.length" class="flex flex-col gap-4 mt-10">
                            <h2 class="text-white font-bold text-xl">Albums & Singles</h2>
                            <AlbumCard :info="albums" :info-type="'album'"></AlbumCard>
                        </div>
                    </div>
...
import { album, artist, track } from '../types/SpotifyTypes.interface';
...
const toSpotify = () => {
    const spotifyUrl = artistInfo.value?.external_urls.spotify
    if (spotifyUrl) {
...
const getTopTracks = async () => {
    loading.value = true;
    try {
        const response = await axios.get(`/artists/${artistId.value}/top-tracks?market=TR`, {
            headers: {
...
const getAlbums = async () => {
    loading.value = true;
    try {
        const response = await axios.get(`/artists/${artistId.value}/albums`, {
            headers: {
                Authorization: authToken
            }
        });
...
const getInfo = async () => {
    ...
    error.value = null
    try {
        const response = await axios.get(`/artists/${artistId.value}`, {
            headers: {
                Authorization: authToken
            }
        });
