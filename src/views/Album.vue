                    ...
                    <div
                        ...
                        <div class="flex lg:flex-row flex-col items-center gap-14">
                            <div class="w-[250px] min-w-[250px] h-[250px] min-h-[250px] rounded-full"
                                :style="{ backgroundImage: `url('${albumInfo?.images[0] ? albumInfo?.images[0].url : 'https://placehold.co/600x600'}')`, backgroundSize: 'cover' }">
                            </div>
                            <div class="flex flex-col gap-4">
                                <h2 class="text-white font-bold text-3xl">{{ albumInfo?.name }}</h2>
                                <div class="flex flex-row flex-wrap gap-2">
                                    <div class="text-white flex flex-row" v-for="artist, counter in albumInfo?.artists"
                                        :key="artist.id">
                                        <RouterLink :to="{ name: 'artist', params: { id: artist.id } }"
                                            class="mr-2 hover:underline min-w-max">{{
                        ...
                        <div v-if="albumTracks?.length" class="flex flex-col gap-4 mt-10">
                                ...
                                <div
                                    ...
                                    <div class="flex flex-row gap-2 items-center">
                                        <img class="w-12 rounded"
                                            :src="albumInfo?.images ? albumInfo.images[0].url : 'https://placehold.co/600x600'"
                                            alt="">
                                        <span v-if="track?.name.length < 40">{{ track?.name }}</span>
                                        <span v-else>{{ track?.name.substring(0, 40) + '...' }}</span>
                                    </div>
                                    <div class="flex flex-row items-center gap-2">
                                        <p class="text-white font-medium text-sm">{{ formatMinute(track.duration_ms) }}</p>
...
const albumId = ref<string | null | string[]>(null);
...
const toSpotify = () => {
    const spotifyUrl = albumInfo.value?.external_urls.spotify
    if (spotifyUrl) {
...
const getInfo = async () => {
    ...
    error.value = null
    try {
        const response = await axios.get(`/albums/${albumId.value}`, {
            headers: {
        ...
        albumTracks.value = response.data.tracks.items
        loading.value = false
    } catch (error) {
