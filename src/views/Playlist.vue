        ...
        <div v-if="!loading && !error" class="w-full flex flex-col overflow-hidden">
            <div class="w-full flex flex-col overflow-hidden">
                <div class="z-10 w-full flex flex-col items-center h-screen py-5">
                    <div
                        class="w-1/2 min-w-[300px] h-screen overflow-y-auto bg-gray-900/80 flex flex-col rounded-md p-4 backdrop-blur-sm">
                        ...
                        <div class="flex lg:flex-row flex-col items-center gap-14">
                            <div class="w-[250px] min-w-[250px] h-[250px] min-h-[250px] rounded-full"
                                :style="{ backgroundImage: `url('${playlistInfo?.images[0] ? playlistInfo?.images[0].url : 'https://placehold.co/600x600'}')`, backgroundSize: 'cover' }">
                            </div>
                            <div class="flex flex-col gap-4">
                                <h2 class="text-white font-bold text-3xl">{{ playlistInfo?.name }}</h2>
                                <p class="text-white" v-if="playlistInfo.description">{{ playlistInfo.description }}</p>
                                <RouterLink :to="{ name: 'user', params: { id: playlistInfo.owner.id } }"
                                    class="text-white hover:underline">
                                    {{ playlistInfo.owner.display_name }}
                                </RouterLink>
                                <button type="button" @click="toSpotify"
                                    class="inline-flex items-center w-fit gap-2 py-2 px-4 border-2 border-green-500 rounded-full bg-green-100">
                                    ...
                                    </svg> Listen on Spotify </button>
                            </div>
                        </div>
                        <div v-if="playlistTracks?.length" class="flex flex-col gap-4 mt-10">
                            <h2 class="text-white font-bold text-xl">Tracks</h2>
                            <div v-for="track in playlistTracks" :key="track.id">
                                <div
                                    class="p-2 bg-gray-900/[0.5] rounded flex flex-row gap-2 items-center justify-between text-white">
                                    <div class="flex flex-row gap-2 items-center">
                                        <img class="w-12 rounded"
                                            :src="track.track?.album.images ? track.track.album.images[0].url : 'https://placehold.co/600x600'"
                                            alt="">
                                        <div>
                                            <span v-if="track.track?.name.length < 40">{{ track?.track.name }}</span>
                                            <span v-else>{{ track?.track.name.substring(0, 40) + '...' }}</span>
                                            <p class="text-sm font-light">{{ track.track.artists[0].name }}</p>
                                        </div>
                                    </div>
                                    <div class="flex flex-row items-center gap-2">
                                        <p class="text-white font-medium text-sm">{{ formatDate(track.added_at) }}</p>
                                        <p class="text-white font-medium text-sm">{{ formatMinute(track.track.duration_ms)
                                        }}</p>

                                        <svg @click="listenOnSpotify(track.external_urls.spotify)"
...
import type { track } from '../types/SpotifyTypes.interface'
import axios from 'axios';

const authToken = sessionStorage.getItem('token');
const playlistId = ref<string | null | string[]>(null);
const playlistInfo = ref<album>()
...
const toSpotify = () => {
    const spotifyUrl = playlistInfo.value?.external_urls.spotify
    if (spotifyUrl) {
...
const getInfo = async () => {
    ...
    error.value = null
    try {
        const response = await axios.get(`/playlists/${playlistId.value}`, {
            headers: {
        ...
        playlistTracks.value = response.data.tracks.items
        loading.value = false
    } catch (error) {
...
const listenOnSpotify = (trackUrl: string) => {
