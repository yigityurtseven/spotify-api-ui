            ...
            <Slide v-for="slide in props.info" :key="slide.id" class="">
                <div class="flex flex-col gap-2 p-4 items-baseline text-left justify-start">
                    <div class="h-[200px] w-[200px] flex flex-col items-end justify-end"
                        :style="{ backgroundImage: `url('${slide?.album.images[1] ? slide?.album.images[1].url : 'https://placehold.co/600x600'}')`, backgroundSize: 'cover' }">
                        <button type="button" @click="toSpotify(slide.album.external_urls.spotify)" class="mb-1 mr-1">
                    ...
                    <p class="text-white font-medium text-sm">{{ formatMinute(slide.duration_ms) }}</p>
                    <p class="text-white font-medium text-sm"> {{ slide.album.album_type.charAt(0).toUpperCase() +
                        slide.album.album_type.slice(1) }} - {{
        formatDate(slide.album.release_date) }}</p>
                </div>
...
import { track } from '../types/SpotifyTypes.interface'
import { ref } from 'vue';
